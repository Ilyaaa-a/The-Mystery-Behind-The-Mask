...
screen main_menu():
    ...
    imagemap:
        ...
        idle "gui/buttons unactive.png"
        hover "gui/buttons active.png"

        hotspot(29, 158, 336, 96) action Start()
        hotspot(29, 357, 423, 88) action ShowMenu("load")
        hotspot(29, 453, 466, 87) action ShowMenu("preferences")
        hotspot(29, 544, 371, 94) action ShowMenu("about")
        hotspot(29, 642, 390, 90) action ShowMenu("help")
        hotspot(29, 735, 380, 95) action ShowMenu("gallery") # будет галерея

        hotspot(29, 925, 329, 87) action Quit(confirm=True)
        
# краивая листающаяся галерея
# screen gallery():
#     tag menu  # Заменяет другие экраны меню
#     add "gui/stars_background.png"  # Фон галереи
...
#         # Проверяем статус разблокировки концовки
#         if persistent.Badend:
#             add gallery_images[current_index]["image"]  # Показываем открытое изображение
#         else:
#             add locked_image  # Показываем замещающее изображение

#     # Подпись под картинкой
#     if persistent.Badend:
#         text gallery_images[current_index]["title"] xalign 0.5 yalign 0.7 color "#782236" size 28
#     else:
#         text "??? (Не открыто)" xalign 0.5 yalign 0.7 color "#999999" size 28
...
#         # Кнопка "Назад"
#         textbutton "Назад" action If(current_index > 0, SetVariable("current_index", current_index - 1)) style "gallery_button"

#         # Кнопка "Вперед"
#         textbutton "Вперед" action If(current_index < len(gallery_images) - 1, SetVariable("current_index", current_index + 1)) style "gallery_button"

#     # Кнопка назад
#     textbutton "Назад" text_color "#782236" text_hover_color "#b12e58" action Return() xalign 0.5 yalign 0.95

screen fullscreen_image(image):
    tag menu
    add image  # Изображение на весь экран
...
screen gallery():
    tag menu  # Чтобы только один экран показывался в момент времени
    add "gui/stars_background.png"  # Фоновое изображение
    ...
    grid 2 2:  # Сетка 2x2
        ...
        vbox:
            xalign 0.5
            yalign 0.5
            spacing 5  # Расстояние между кнопкой и текстом
            add g.make_button("best_end", "images/gallery_posters/unlocked gallery4.png", xalign=0.5, yalign=0.5, hover_border="hovered_gallery")
            if persistent.Best_end:
                textbutton "Лучшая концовка":
                    text_color "#782236"
                    text_hover_color "#b12e58"
                    action Show("fullscreen_image", image="images/gallery_posters/gallery full 4.png")
                    xalign 0.5
                    yalign 0.95
            else:
                text "???" xalign 0.5 color "#782236"

        # Второй элемент с подписью
        vbox:
            xalign 0.5
            yalign 0.5
            spacing 5
            add g.make_button("good_mar_end", "images/gallery_posters/unlocked gallery2.png", xalign=0.5, yalign=0.5, hover_border="hovered_gallery")
            if persistent.Good_Mar_end:
                textbutton "Супер-Кот спас Маринетт":
                    text_color "#782236"
                    text_hover_color "#b12e58"
                    action Show("fullscreen_image", image="images/gallery_posters/gallery full 2.png")
                    xalign 0.5
                    yalign 0.95
            else:
                text "???" xalign 0.5 color "#782236"
        # Третий элемент с подписью
        vbox:
            xalign 0.5
            yalign 0.5
            spacing 5
            add g.make_button("good_adr_end", "images/gallery_posters/unlocked gallery1.png", xalign=0.5, yalign=0.5, hover_border="hovered_gallery")
            if persistent.Good_Adr_end:
                textbutton "Маринетт спасла Супер-Кота":
                    text_color "#782236"
                    text_hover_color "#b12e58"
                    action Show("fullscreen_image", image="images/gallery_posters/gallery full 1.png")
                    xalign 0.5
                    yalign 0.95
            else:
                text "???" xalign 0.5 color "#782236"

        # Четвертый элемент с подписью
        vbox:
            xalign 0.5
            yalign 0.5
            spacing 5
            add g.make_button("bad_end", "images/gallery_posters/unlocked gallery3.png", xalign=0.5, yalign=0.5, hover_border="hovered_gallery")
            if persistent.Bad_end:
                textbutton "Мираж победила":
                    text_color "#782236"
                    text_hover_color "#b12e58"
                    action Show("fullscreen_image", image="images/gallery_posters/gallery full 3.png")
                    xalign 0.5
                    yalign 0.95
...
# выбор за кого играть (Маринетт или Супер-Кот)
screen choose_character():
    tag menu
    add "images/backs/bg school.jpg"

    # Кнопки на изображении
    imagebutton:
        idle "images/char/mar hi.png"
        hover "images/buttons/marinett_hover.png"
        action Jump("marinett")
        focus_mask True
        xalign 0.2  # Позиция по горизонтали (20% от ширины экрана)
        yalign 1.0  # Позиция по вертикали (100% от высоты экрана)

    imagebutton:
        idle "images/char/adr normal.png"
        hover "images/buttons/adrian_hover.png"
        action Jump("adrian")
        focus_mask True
        xalign 0.8
        yalign 1
